...
## Purpose
Defines the detail/abstract render posture: a user-controlled slider that
lets block count/nesting depth be chosen rather than being an accidental
side effect of per-frame pixel geometry, plus the hover-only preview that
lets a user peek inside a collapsed directory block without drilling in.
