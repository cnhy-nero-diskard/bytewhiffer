...
- [ ] 1.1 Add a new `theme.rs` color rule for on-block label text: an
      alpha-blended black overlay (not a fixed gray constant) that darkens
      whatever color/lightness sits beneath it.
- [ ] 1.2 Add a `theme.rs` unit test asserting the new label color darkens a
...
- [ ] 2.1 Add a size-label fit-gate helper distinct from the existing
...
- [ ] 2.2 Wire that gate into `draw_children`'s flat branch: paint the
      block's `util::format_size(child.size)` in the top-right corner when
      the gate passes, using the new label color from 1.1.
- [ ] 2.3 Add a directory-tray-specific version of the gate that measures
...
- [ ] 2.4 Wire that tray gate into `draw_tray_shell` (`app.rs:1861`): paint
      the tray's total size in the header's top-right corner when it
      passes, using the new label color from 1.1.
- [ ] 2.5 Apply the new label color from 1.1 to the two existing name-label
...
- [ ] 3.1 Add a `total_size: u64` field to `InsightsData` (`app.rs:358`),
      set from `view.size` in `refresh_insights` (`app.rs:1124`) alongside
      the existing `ext_totals`/`leaderboard` fields.
- [ ] 3.2 Add a new flat neutral (green) bar-fill color constant to
      `theme.rs`.
- [ ] 3.3 Add a row bar-paint helper alongside `swatch()` (`app.rs:1597`):
...
- [ ] 3.4 Wire the bar helper into the "File types" section's rows
      (`app.rs:1170`-`1183`), sizing each bar to
      `ext_size as f64 / total_size as f64`.
- [ ] 3.5 Wire the bar helper into the "Biggest items" leaderboard's rows
