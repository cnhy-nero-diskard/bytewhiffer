...
scanned" and "bytes scanned so far," and that every conforming engine updates
in a way consistent with monotonically increasing completion, whether or not
that engine can report smooth incremental partial trees.
...
- **THEN** the reported files-scanned and bytes-scanned counters never
  decrease between polls
