...
- [x] 1.1 Add a new muted saturation/value band for directory frames in
      ...
      alpha/lerp amount.
- [x] 1.2 Add a function that hashes a directory's name (reusing `fnv1a`)
      ...
      color.
- [x] 1.3 Derive the frame's faint fill tint from the same border color
      (e.g. a low-alpha or heavily-lightened-toward-`BG` variant), replacing
      `TRAY_FILL`.
- [x] 1.4 Update `tray_header_color` (or its replacement) to use the new
      per-name hue instead of the fixed `DIR_HUE`, keeping `depth_shift`
      applied on top as before.
- [x] 1.5 Remove `TRAY_FILL` and `tray_inset_shadow` now that the inset-
      shadow mesh under the header is dropped in favor of the border+tint
      frame (per design.md's elevation-mesh decision).
- [x] 1.6 Update/add unit tests: same directory name always yields the same
...
- [x] 2.1 Implement a chain-walk helper that, given a `Node`, follows
      ...
      is a file.
- [x] 2.2 Wire the chain-walk into `draw_children` before the existing
...
- [x] 3.3 Re-check `MIN_NEST_AREA`/`MIN_NEST_SIDE`/`MAX_DEPTH` constants
      still make sense against the new (smaller) padding — adjust if the
      reduced inset changes what counts as "big enough to nest."

## 4. Perf spike parity

- [x] 4.1 Update `collect_bench_blocks`/`build_elevated_shapes` (the
      ...
      cost.
- [x] 4.2 Re-run `--debug-perf` against both existing scenes
...
- [x] 5.1 `cargo test` — scanner/treemap/theme/util unit tests pass,
      including the new/updated theme tests from section 1.
- [x] 5.2 Build via the documented Windows GNU toolchain recipe
      (`cargo +stable-x86_64-pc-windows-gnu build --release` with WinLibs
      mingw64 on PATH) and confirm it compiles clean.
- [x] 5.3 `--debug-screenshot-drill` (or `--debug-screenshot`) against a
...
- [x] 5.4 `--debug-screenshot` against a branchy/dense scan (e.g. a
